...
public:
    ...
    int x, y;   //坐标（相对于根节点(0,0)）的偏移量
    vector<int> width, height;   //放于第i层时的宽和高
    ...
    vector<vector<pair<int,int>>> pins_coor;    //pins_coor[i][j]表示第i层引脚j的相对坐标（相对模块坐标），编号 j 从 0 到 pins_num-1
    ...
    void add_layer_info(int l, int w, int h)       //添加l层的宽度w和高度h信息
    {
        assert(l < layersize && w > 0 && h > 0);
        width[l] = w;
        height[l] = h;
    }
    void add_pins_info(int l, vector<pair<int,int>> pins_coor_thislayyer) //添加l层的引脚
    {
    ...
    pair<int,int> get_pin_coor(int pin_id)  //引脚pin_id的坐标，每顺时针旋转90°，坐下角坐标就逆时针转移到下一个顶点
    {
        pair<int,int> coor; //相对于初始点的坐标
        pair<int,int> modify_offset; //修正偏移量，旋转后左下坐标会逆时针转移到下一个顶点
        int x0 = pins_coor[layer][pin_id].first;
        int y0 = pins_coor[layer][pin_id].second;
